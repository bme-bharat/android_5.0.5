...
const EditProduct = () => {
    ...
    const openGallery = async () => {
        try {
            ...
            // Add to newImages (for images picked in this session)
            setNewImages((prev) => {
                const totalImages = images.length + prev.length;
                if (totalImages >= 4) {
                    showToast("You can only select up to 4 images", 'error');
                    return prev;
                }

                const updatedNewImages = [...prev, { ...file, uri: resizedImage.uri, size: resizedImage.size }];
                console.log('Updated newImages:', updatedNewImages);
    ...
    const selectPDF = async () => {
        try {
            ...
            // ✅ Store the PDF in newFiles, replacing previous if any
            setNewFiles([{ uri: file.uri, name: file.name, size: file.size, mime: mimeType }]);
            console.log('Selected PDF stored in newFiles:', file);
